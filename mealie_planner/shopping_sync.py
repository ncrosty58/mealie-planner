...
from .config import _SHOPPING_LIST_SYNC_SKILL_DEFINITION, ACTIVE_LIST_ID, FAMILY_DIETARY_RULES_PROMPT, STAPLES_LIST_ID
from .models import CompiledShoppingListWrapper
...
class ShoppingListSync:
    ...
    def sync_staples_only(self, low_staples_ids, list_id=ACTIVE_LIST_ID) -> bool:
        ...
        logger.info("[Sync] Performing fast staples-only update...")
        try:
            staples = self.client.get_shopping_list_items_for_list(STAPLES_LIST_ID)
            ...
            logger.error(f"Error during fast staples sync: {e}")
            return False

    def sync_shopping_list(self, start_date_str, end_date_str, low_staples_ids=None, progress_callback=None, freezer_items="", list_id=ACTIVE_LIST_ID) -> bool:
        """Non-destructive sync using AI to perform semantic matching and checkmark/ID retention."""
        low_staples_ids = low_staples_ids or []
        logger.info(f"Starting non-destructive sync for {start_date_str} to {end_date_str}...")
        if progress_callback:
            progress_callback("Sync started...", 90)
        ...
        try:
            ...
            if progress_callback:
                progress_callback("Analyzing current progress...", 93)
            ...
            logger.info(f"[Sync] Bulk fetching details for {len(recipe_ids_to_fetch)} unique recipes.")
                ...
                if rid and rid in details_map:
                    ...
                    if r_details:
                        for ing in r_details.get('recipeIngredient', []):
                            ...
                            if txt.strip():
                                raw_recipe_ingredients.append(txt.strip())

            # 3. Call AI Skill
            if progress_callback:
                progress_callback("Generating optimized list...", 96)
            ...
            payload = {
                ...
                "available_labels": [label['name'] for label in all_labels],
            ...
            if progress_callback:
                progress_callback("Merging changes...", 98)
            ...
            for i in active_items:
                if i['id'] not in matched_ids:
                    ...
                    if is_manual:
                        logger.info(f"[Sync] Preserving unmatched manual item: {i['note']}")
                    elif is_low_staple:
                        logger.info(f"[Sync] Preserving unmatched low staple: {i['note']}")
            ...
            for s in staples:
                if s['id'].replace('-', '').lower() in low_ids_clean:
                    ...
                    if s_norm not in processed_notes_norm:
                        logger.warning(f"[Sync] Adding missing low staple in Python: {s['note']}")
                        to_add.append({
                            "shoppingListId": list_id,
                            "note": s['note'],
                            "quantity": s.get('quantity', 1.0),
            ...
            if to_delete:
                self.client.delete_shopping_list_items_bulk(to_delete)
            if to_update:
            ...
            logger.error(f"Error during AI shopping list sync: {e}")
            if progress_callback:
                progress_callback(f"⚠️ Shopping list sync failed: AI error ({e}). The shopping list was not updated.", 100)
            return False

def sync_shopping_list(start_date_str, end_date_str, low_staples_ids=None, progress_callback=None, freezer_items=""):
